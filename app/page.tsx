...
export default function Home() {
  ...
  const groundingVideo = videoRecords[groundingIndex];
  const activeGrounding = groundingVideo.qwen.segments.find((segment) => groundingTime >= segment.start_sec && groundingTime < segment.end_sec);
  const groundingProgress = Math.min(100, Math.max(0, (groundingTime / Math.max(groundingVideo.duration, 0.1)) * 100));
  ...
  return (
    <main className="app-shell">
      ...
      {tab === "grounding" && <section className="section-block view-block grounding-view"><div className="section-heading"><div><div className="eyebrow">GLOBAL TASK / SEMANTIC SUBTASK GROUNDING</div><h2>每个 subtask 都服务于全局任务</h2></div><span className="muted">QWEN DENSE · TEMPORAL SPANS</span></div><div className="grounding-note"><strong>标注原则：</strong>先用全局任务语义拆出“接近、对齐、抓取、搬运、放置”等可执行 subtask，再给每个 subtask 一个时间区间。高 FPS 窗口只用于边界复核，不生成脱离任务语义的 caption。</div><div className="grounding-layout"><div className="grounding-list" aria-label="语义 grounding 视频列表">{videoRecords.map((item, index) => <button key={item.id} className={groundingIndex === index ? "grounding-item selected" : "grounding-item"} onClick={() => { setGroundingIndex(index); setGroundingTime(0); }}><span className="fine-item-top"><span className="sample-id">{item.id}</span><span>{item.qwen.segments.length} subtasks</span></span><span className="fine-item-task">{item.task}</span><span className="fine-item-meta">{item.duration.toFixed(1)}s · success GT · Qwen semantic</span></button>)}</div><article className="grounding-stage"><video className="video-player" controls preload="metadata" src={groundingVideo.src} aria-label={`${groundingVideo.id} semantic grounding video`} onTimeUpdate={(event) => setGroundingTime(event.currentTarget.currentTime)} onLoadedMetadata={() => setGroundingTime(0)}><track kind="captions" src={groundingVideo.src.replace(".mp4", ".vtt")} srcLang="en" label="Semantic subtask grounding" /></video><div className="grounding-task"><span className="fine-now-label">GLOBAL TASK</span><strong>{groundingVideo.task}</strong><span>GT success · {groundingVideo.qwen.segments.length} semantic subtasks</span></div><div className="fine-progress-meta"><span>{formatPreciseTime(groundingTime)} / {formatPreciseTime(groundingVideo.duration)}</span><span>{activeGrounding ? `Subtask ${groundingVideo.qwen.segments.indexOf(activeGrounding) + 1}/${groundingVideo.qwen.segments.length}` : "No active subtask"}</span></div><div className="grounding-track" role="progressbar" aria-valuemin={0} aria-valuemax={groundingVideo.duration} aria-valuenow={groundingTime}><span className="fine-fill" style={{ width: `${groundingProgress}%` }} />{groundingVideo.qwen.segments.map((segment, index) => <span className={activeGrounding === segment ? "grounding-span active" : "grounding-span"} key={`${groundingVideo.id}-grounding-${index}`} style={{ left: `${(segment.start_sec / groundingVideo.duration) * 100}%`, width: `${((segment.end_sec - segment.start_sec) / groundingVideo.duration) * 100}%` }} />)}</div><div className="grounding-active">{activeGrounding ? <><div><span className="fine-now-label">ACTIVE SEMANTIC SUBTASK</span><strong>{activeGrounding.start_sec.toFixed(2)}–{activeGrounding.end_sec.toFixed(2)}s</strong></div><p>{activeGrounding.action}</p><small>全局语义 grounding · 可用于动作计划/奖励对齐/人工审核</small></> : <p>拖动视频时间轴查看当前 subtask。</p>}</div><div className="grounding-subtasks">{groundingVideo.qwen.segments.map((segment, index) => <div className={activeGrounding === segment ? "grounding-row active" : "grounding-row"} key={`${groundingVideo.id}-row-${index}`}><span className="grounding-index">{String(index + 1).padStart(2, "0")}</span><span className="fine-label-time">{segment.start_sec.toFixed(2)}–{segment.end_sec.toFixed(2)}s</span><span>{segment.action}</span></div>)}</div></article></div></section>}
