...
export default function Home() {
  ...
  return (
    <main className="app-shell">
      ...
      {tab === "fine" && <section className="section-block view-block fine-view"><div className="section-heading"><div><div className="eyebrow">HIGH FPS / LOCAL SLIDING WINDOWS</div><h2>看 20 FPS 的 0.25 秒 caption 候选</h2></div><span className="muted">TIMELENS2 · EXPERIMENTAL</span></div>{fineLoadError ? <div className="fine-empty">高 FPS 数据文件未加载，请确认 `public/high_fps_data.json` 存在。</div> : !fineVideo ? <div className="fine-empty">正在加载 532 个短窗候选…</div> : <><div className="fine-summary"><div><span>WINDOWS</span><strong>{fineRecords.reduce((total, item) => total + item.labels.length, 0)}</strong><small>10 videos</small></div><div><span>SAMPLING</span><strong>{fineVideo.fps.toFixed(0)} FPS</strong><small>0.05s grid</small></div><div><span>CONTEXT</span><strong>{fineVideo.windowSec.toFixed(2)}s</strong><small>overlapping</small></div><div><span>STRIDE</span><strong>{fineVideo.strideSec.toFixed(2)}s</strong><small>subtitle step</small></div></div><div className="fine-note">这是高分辨率候选字幕：窗口之间有重叠，静止画面可能重复，暂不替换已验证的 Qwen dense 主字幕。</div><div className="fine-layout"><div className="fine-list" aria-label="高 FPS 视频列表">{fineRecords.map((item, index) => <button key={item.id} className={fineIndex === index ? "fine-item selected" : "fine-item"} onClick={() => { setFineIndex(index); setFineTime(0); }}><span className="fine-item-top"><span className="sample-id">{item.id}</span><span>{item.labels.length} windows</span></span><span className="fine-item-task">{item.task}</span><span className="fine-item-meta">{item.duration.toFixed(1)}s · {item.fps.toFixed(0)} FPS · {item.strideSec.toFixed(2)}s step</span></button>)}</div><article className="fine-stage"><video className="video-player" controls preload="metadata" src={fineVideo.src} aria-label={`${fineVideo.id} high FPS video`} onTimeUpdate={(event) => setFineTime(event.currentTarget.currentTime)} onLoadedMetadata={() => setFineTime(0)}><track kind="captions" src={`/videos/${fineVideo.id}.vtt`} srcLang="en" label="Qwen dense reference" /></video><div className="fine-now"><div><span className="fine-now-label">ACTIVE WINDOW</span><strong>{fineActive ? `${formatPreciseTime(fineActive.start)} – ${formatPreciseTime(fineActive.end)}` : formatPreciseTime(fineTime)}</strong></div><div className="fine-now-phase">{fineActive?.phase ?? "—"}</div><p>{fineActive?.caption ?? "拖动视频进度条查看短窗 caption"}</p></div><div className="fine-progress-meta"><span>{formatPreciseTime(fineTime)} / {formatPreciseTime(fineVideo.duration)}</span><span>{fineVideo.labels.length} windows</span></div><div className="fine-track" role="progressbar" aria-valuemin={0} aria-valuemax={fineVideo.duration} aria-valuenow={fineTime}><span className="fine-fill" style={{ width: `${fineProgress}%` }} />{fineVideo.labels.map((label, index) => <span className={fineActive === label ? "fine-marker active" : "fine-marker"} key={`${fineVideo.id}-${index}`} style={{ left: `${(label.start / fineVideo.duration) * 100}%`, width: `${((label.end - label.start) / fineVideo.duration) * 100}%` }} />)}</div><div className="fine-label-list">{fineVideo.labels.map((label, index) => <div className={fineActive === label ? "fine-label active" : "fine-label"} key={`${fineVideo.id}-label-${index}`}><span className="fine-label-time">{formatPreciseTime(label.start)}–{formatPreciseTime(label.end)}</span><span className="fine-label-phase">{label.phase}</span><span>{label.caption}</span></div>)}</div></article></div></>}</section>}
