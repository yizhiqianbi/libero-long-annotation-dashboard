...
const models: Record<ModelKey, {
  ...
  qwen: {
    ...
    gpu: "1 × H200",
    memory: "约 54 GB BF16",
    note: "唯一能同时输出成功状态、失败原因和动作语义的模型。本批 10 条均为数据集成功演示，Qwen 识别出 8 条成功。",
  },
  timelens2: {
    name: "TimeLens2-8B",
    short: "TimeLens2",
    ...
    <main className="app-shell">
      ...
      {tab === "overview" && <>
        <section className="headline-grid">
          <article className="headline-card accent"><div className="card-kicker">SEMANTIC READ</div><div className="headline">8<span className="unit">/ 10</span></div><p>Qwen 对数据集成功标签的识别结果；两个 false 是语义误判，不是原始演示失败。</p><div className="card-foot"><span>QWEN SUCCESS ACCURACY</span><strong>80%</strong></div></article>
        ...
        <section className="two-col section-block"><div><div className="eyebrow">02 / KEY FINDING</div><h2>成功演示也会被语义模型误判</h2><p className="body-copy">10 条回放的 ground truth 全部是成功，但 Qwen 在两个长任务上认为末步没有完成。TimeLens2 和 TimeLens 都返回了合法区间，说明时间定位与任务成败是两个独立维度。</p><div className="quote"><span>“</span><p>先保留数据集 success label，再把 caption 与 temporal span 作为可解释证据。</p></div></div><div className="bar-panel"><div className="panel-label">SUCCESS DEMOS · 10 TOTAL</div><div className="bar-row"><span>Qwen judged success</span><div className="bar-track"><i style={{ width: "80%", background: models.qwen.color }} /></div><strong>8</strong></div><div className="bar-row"><span>TimeLens2 valid span</span><div className="bar-track"><i style={{ width: "100%", background: models.timelens2.color }} /></div><strong>10</strong></div><div className="bar-row"><span>TimeLens valid span</span><div className="bar-track"><i style={{ width: "100%", background: models.timelens.color }} /></div><strong>10</strong></div><div className="panel-note">TimeLens 的 10 / 10 是输出格式与区间合法性，不代表它判断了任务成功。</div></div></section>
      </>}

      {tab === "quality" && <section className="section-block view-block"><div className="section-heading"><div><div className="eyebrow">QUALITY / 10 LIBERO-LONG DEMOS</div><h2>能力拆分比单一排名更诚实</h2></div><span className="muted">SAME SUCCESS-ONLY BATCH</span></div><div className="comparison-table"><div className="table-row table-head"><span>MODEL</span><span>ROLE</span><span>PARSE</span><span>READOUT</span><span>LATENCY</span></div>{(Object.keys(models) as ModelKey[]).map(key => <div className="table-row" key={key}><span className="table-model"><i style={{ background: models[key].color }} />{models[key].name}</span><span>{models[key].role}</span><span>{models[key].parse}</span><span className={key === "qwen" ? "good" : "warn"}>{key === "qwen" ? "8 / 10 success" : "10 / 10 valid spans"}</span><span className="mono">{models[key].latency}</span></div>)}</div><div className="quality-notes"><article><span className="note-num">A</span><h3>Qwen / semantic</h3><p>8 / 10 与数据集 success label 一致；两个 false negative 说明长任务末步需要人工复核。</p></article><article><span className="note-num">B</span><h3>TimeLens / temporal</h3><p>三种模型结果在视频页逐条并排展示。TimeLens 只回答查询事件的时间范围，不输出任务成败。</p></article><article><span className="note-num">C</span><h3>Gold set / next</h3><p>当前成功演示适合检查误报；补充失败演示和人工动作边界后，才能计算完整 F1 与 temporal mIoU。</p></article></div></section>}

      {tab === "videos" && <section className="section-block view-block video-view"><div className="section-heading"><div><div className="eyebrow">VIDEO + THREE MODELS / 10 RECORDS</div><h2>逐条回放视频，并对照三个模型</h2></div><span className="muted">SUCCESS-ONLY · SINGLE CARD EACH</span></div><div className="video-layout"><div className="video-list" aria-label="视频列表">{videoRecords.map((item, index) => <button key={item.id} className={videoIndex === index ? "video-item selected" : "video-item"} onClick={() => { setVideoIndex(index); setVideoTime(0); }}><span className="video-item-top"><span className="sample-id">{item.id}</span><span className="video-status success">GT 成功</span></span><span className="video-item-task">{item.task}</span><span className="video-item-meta">{item.scenario} · {item.duration.toFixed(1)}s · {item.episodeId}</span></button>)}</div><article className="video-stage"><video className="video-player" controls preload="metadata" src={video.src} aria-label={`${video.id} video`} onTimeUpdate={(event) => setVideoTime(event.currentTarget.currentTime)} onLoadedMetadata={() => setVideoTime(0)}><track kind="captions" src={video.src.replace(".mp4", ".vtt")} srcLang="en" label="Qwen caption" /></video><div className="video-progress-wrap" aria-label="视频播放进度"><div className="video-progress-meta"><span>{formatTime(videoTime)} / {formatTime(video.duration)}</span><span>{activeSegment ? `动作 ${video.qwen.segments.indexOf(activeSegment) + 1}/${video.qwen.segments.length}` : "caption"}</span></div><div className="video-progress-track" role="progressbar" aria-valuemin={0} aria-valuemax={video.duration} aria-valuenow={videoTime}><span className="video-progress-fill" style={{ width: videoProgress + "%" }} />{video.qwen.segments.map((segment) => <span className="video-progress-marker" key={segment.start_sec} style={{ left: (segment.start_sec / video.duration * 100) + "%" }} />)}</div></div><div className="subtitle-bar" aria-live="polite"><span className="subtitle-time">{formatTime(videoTime)}</span><span className="subtitle-copy">{activeSegment?.action ?? video.qwen.caption}</span></div><div className="video-caption"><div className="detail-top"><span className="sample-id">{video.id} · {video.scenario}</span><span className="video-status success">DATASET GT 成功</span></div><h3>{video.task}</h3><div className="caption-label">OBSERVED CAPTION · QWEN</div><p className="caption-text">{video.qwen.caption}</p>{video.qwen.reason && <p className="caption-reason"><strong>Qwen 误判原因：</strong>{video.qwen.reason}</p>}<div className="caption-meta"><span>VIDEO DURATION <strong>{video.duration.toFixed(1)} s</strong></span><span>EPISODE <strong>{video.episodeId}</strong></span><span>QWEN LATENCY <strong>{video.qwen.latency.toFixed(3)} s</strong></span></div><div className="caption-label">ATOMIC SEGMENTS · QWEN</div><div className="caption-segments">{video.qwen.segments.map((segment, index) => <div className="caption-segment" key={`${video.id}-${index}`}><span className="segment-time">{segment.start_sec.toFixed(1)}–{segment.end_sec.toFixed(1)}s</span><span>{segment.action}</span></div>)}</div><div className="caption-label">MODEL RESULT COMPARISON</div><div className="model-result-grid"><article className="model-result-card"><div className="model-result-head"><i style={{ background: models.qwen.color }} /><strong>Qwen</strong><span>{video.qwen.success ? "success" : "false"}</span></div><div className="model-result-value">{video.qwen.success ? "模型判断完成" : "模型判断未完成"}</div><div className="model-result-meta">semantic · {video.qwen.latency.toFixed(3)} s</div></article>{(["timelens2", "timelens"] as const).map((key) => <article className="model-result-card" key={key}><div className="model-result-head"><i style={{ background: models[key].color }} /><strong>{models[key].short}</strong><span>temporal</span></div><div className="model-result-value">{video[key].spans.map((span) => `${span.start.toFixed(1)}–${span.end.toFixed(1)}s`).join(" · ")}</div>{video[key].spans.map((span, index) => <div className="interval-track" key={index}><i style={{ left: (span.start / video.duration * 100) + "%", width: ((span.end - span.start) / video.duration * 100) + "%", background: models[key].color }} /></div>)}<div className="model-result-meta">event span · {video[key].latency.toFixed(3)} s</div></article>)}</div></div></article></div></section>}

      {tab === "samples" && <section className="section-block view-block"><div className="section-heading"><div><div className="eyebrow">SAMPLE EXPLORER / SUCCESS-ONLY</div><h2>从原始视频到三模型标签</h2></div><span className="muted">SELECT A CASE</span></div><div className="sample-layout"><div className="sample-list">{videoRecords.slice(0, 4).map((item, index) => <button key={item.id} className={sampleIndex === index ? "sample-item selected" : "sample-item"} onClick={() => setSampleIndex(index)}><span className="sample-id">{item.id}</span><span>{item.task}</span><b className="locate">成功</b></button>)}</div><article className="sample-detail"><div className="detail-top"><span className="sample-id">{sample.id}</span><span className="locate">DATASET 成功</span></div><h3>{sample.task}</h3><div className="detail-meta"><span>QWEN</span><strong>{sample.qwen.success ? "判断成功" : "判断失败"}</strong><span>TL2</span><strong>{sample.timelens2.spans[0] ? `${sample.timelens2.spans[0].start.toFixed(1)}–${sample.timelens2.spans[0].end.toFixed(1)}s` : "—"}</strong><span>TL</span><strong>{sample.timelens.spans[0] ? `${sample.timelens.spans[0].start.toFixed(1)}–${sample.timelens.spans[0].end.toFixed(1)}s` : "—"}</strong></div><p className="reason">{sample.qwen.caption}</p><div className="timeline-label">QWEN ATOMIC SEGMENTS</div><div className="segments">{sample.qwen.segments.map((segment, index) => <div className="segment" key={index}><span className="segment-dot" />{segment.start_sec.toFixed(1)}–{segment.end_sec.toFixed(1)}s - {segment.action}</div>)}</div></article></div></section>}
